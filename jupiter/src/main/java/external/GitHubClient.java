...
public class GitHubClient {
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			...
			// streamReader按个数读，bufferedReader按行数读
			// streamReader要是有1001个，按500个读，最后一个会报错
	...
	private List<Item> getItemList(JSONArray array) {
		...
		for (int i = 0; i < array.length(); i++) {
			...
			// filter
			// 可将builder setter return 改为ItemBuilder (chaining)
			// builder.setItemId(getStringFieldOrEmpty(object, "id")).setName(getStringFieldOrEmpty(object, "title"));
